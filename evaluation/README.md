...
如果当前行或下一行的 `close` 缺失，对应 `y_ret_1d` 保持 NaN，官方脚本会在 Rank IC
和收益计算中自动过滤。
...
入口会先检查列名、行数、重复键、缺失/无限预测以及完整键集合，再调用官方
...
但如果比赛规则要求每个交易日只能使用当时可见数据，那么直接在这些标签上反复调参会
产生测试集未来信息泄漏，得到的分数不能代表严格样本外能力。提交前请由团队确认该用法
符合比赛规则；报告中应明确标注为“测试期事后重构评测”。
